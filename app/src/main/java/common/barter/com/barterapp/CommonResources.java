...
public class CommonResources {
    ...
    public void navigateToPostAd(FragmentManager fragmentManager, String cameFrom) {
        ...
        bundle.putString(MessagesString.CAME_FROM,cameFrom);
    ...
    public void navigateToPostDetails(FragmentManager fragmentManager, NewPost post, String calledFor) {
        Fragment fragment = null;
        fragment = new PostDetailsFragment();
        Bundle bundle = new Bundle();
        bundle.putString(MessagesString.NAV_MODE, calledFor);
    ...
    public static String getStaticURL() {
        String staticURL = "http://192.168.0.104:80/"; //URL
        //String staticURL = "http://49.205.72.242:80/" ;

        return staticURL;
    }

    public static String getImageURL(long postId,int counter) {
        String url = CommonResources.getStaticURL() + MessagesString.IMAGE_LOCATION + "/" + postId + "_"+counter;
        return url;
    }
    public static String getPrimaryImageURL(long postId) {
        return getImageURL(postId,1);
    }
    public static String getLocalitiesURL(String location) {
        StringBuilder URL = new StringBuilder();
        location = location.replace(" ","%20");
        URL.append(getStaticURL()).append("city_jsons/").append(location).append(".json");
        return URL.toString();
    }
    ...
    public static boolean isLoggedIn(Context context, String errorMsg) {
        String uniqueid = LoginDetails.getInstance().getUserid();
        if (uniqueid == null || "".equalsIgnoreCase(uniqueid)) {
            CommonUtil.flash(context, errorMsg);
            return false;
        }
        return true;
    }
